...
export function BundleWizardDialog({
  ...
  const getModeDescription = (mode: string) => {
    switch (mode) {
      ...
      case "launchSnipe":
        return "Combined launch and snipe strategy. Launches tokens while simultaneously setting up snipes for optimal entry. Best for competitive markets."
      default:
        return ""
    }
  }
          ...
          <div className="py-4 space-y-4">
            ...
            <RadioGroup value={bundleData.mode} onValueChange={handleModeChange} className="space-y-3">
              ...
              <div
                key="launchSnipe"
                className={`flex items-start space-x-3 border border-gray-800 rounded-md p-3 cursor-pointer hover:bg-gray-800/30 ${
                  bundleData.mode === "launchSnipe" ? "bg-amber-900/20 border-amber-600/50" : "bg-[#1A1A1A]"
                }`}
                onClick={() => handleModeChange("launchSnipe")}
              >
                <RadioGroupItem id="launchSnipe" value="launchSnipe" className="border-gray-600 text-amber-600 mt-1" />
                <div className="flex-1">
                  <Label htmlFor="launchSnipe" className="text-sm text-[#ECF1F0] cursor-pointer">
                    Launch+Snipe
                  </Label>
                  <p className="text-xs text-gray-400 mt-1">{getModeDescription("launchSnipe")}</p>
                </div>
              </div>
            </RadioGroup>
          </div>
