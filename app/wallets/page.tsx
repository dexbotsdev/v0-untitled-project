...
import { Zap, Plus, Trash2, RefreshCw, Send, Cpu, Repeat } from "lucide-react"
...
export default function WalletsPage() {
  ...
  const handleWsolConversion = () => {
    // Get selected wallets or all wallets if none selected
    const selectedWallets = generatedWallets.filter((wallet) => wallet.selected)
    const walletsToConvert = selectedWallets.length > 0 ? selectedWallets : generatedWallets
    ...
    setTimeout(() => {
      ...
      // Update the wallet balances (in a real app, this would be an API call)
      // If converting to WSOL, deduct a small fee
      const newMode = !isWsolMode
      const conversionFee = newMode ? 0.0001 : 0 // Small fee when converting to WSOL

      setGeneratedWallets((wallets) =>
        wallets.map((wallet) => {
          if (walletsToConvert.some((w) => w.id === wallet.id)) {
            return {
              ...wallet,
              nativeBalance: Math.max(0, wallet.nativeBalance - conversionFee),
            }
          }
          return wallet
        }),
      ...
      toast({
        title: `Conversion to ${newMode ? "WSOL" : "SOL"} Complete`,
        description: `Successfully converted ${walletsToConvert.length} wallets from ${newMode ? "SOL to WSOL" : "WSOL to SOL"}${conversionFee > 0 ? ` (fee: ${conversionFee} SOL per wallet)` : ""}`,
      })
    }, 800)
  }
  ...
  return (
    <>
      <Header2 title="Wallet Accounts Management" subtitle="Create and manage volume bots for your tokens" />
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between bg-[#11111D] p-2 gap-2">
                  ...
                  <div className="flex space-x-2">
                    ...
                    >
                      ...
                      {isConverting ? "Converting..." : isWsolMode ? "WSOL → SOL" : "SOL → WSOL"}
                    </Button>
                  </div>
                </div>
              </div>
                            ...
                            {generatedWallets.map((wallet) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-2 text-right text-[#ECF1F0]">
                                  {wallet.nativeBalance.toFixed(4)} {isWsolMode ? "WSOL" : "SOL"}
                                </td>
                                <td className="px-4 py-2">
                                  <div className="flex items-center space-x-2">
                                    <Input
